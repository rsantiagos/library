<template>
  <div class="v-table">
    ...
    <table>
      <thead>
        <tr>
          <th v-for="(column, ind) in dataTable.columns" :key="ind"
              @click="sortBy(column)"
      ...
      <tbody>
         <tr v-for="(row, kke) in pagination" :key="kke" track-by="$index">
          <td v-for="(item, key) in dataTable.columns" :key="key + kke" @click="editField(row[item.value], item.value)">
            <span v-if="!row[item.value].editing">
              <template v-if="isButton(item.value)">
                <button type="button"
                        v-for="(button, ind) in row[item.value].value"
                        :key="ind"
                        :class="button.class"
                        @click="button.func($event, item.value, button)">{{button.text}}</button>
              </template>
              <template v-else>
                <template v-if="isHTML(item.value)"><div v-html="row[item.value].value"></div></template>
                <template v-else>{{row[item.value].value}}</template>
              </template>
            </span>
            <template v-if="isEditable(row[item.value], item.value)">
    ...
    <div class="v-table-footer">
      ...
      <div class="v-table-footer-page" v-if="lastPage !== 1">
        ...
        <a class="v-table-footer-page-btn" href="javascript:;"
           ...
           v-for="page in centerPartPage" :key="page">{{page + 1}}</a>
        <span v-if="lastPage > 10 && lastPage - currentPage > 5">...</span>
        <a class="v-table-footer-page-btn" href="javascript:;"
           :class="{current: currentPage === page + 1}"
           @click="togglePage(page + 1)"
           v-for="page in lastPartPage" :key="page">{{page + 1}}</a>
        <a class="v-table-footer-page-btn" href="javascript:;"
...
<script>
// reference: https://github.com/galenyuan/vue-datatable
export default {
  name: 'Datatable',
  props: ['dataTable'],
  data () {
  ...
  methods: {
    ...
    filterRows (rows, options, currentPage) {
      ...
      if (this.searchBy !== '') {
        rows = rows.filter((row) => {
          let r = false
          for (let key in row) {
            if (row[key].value && row[key].value.toString().toLowerCase().indexOf(this.searchBy.toLowerCase()) !== -1) {
              r = true
            }
          }
          return r
    ...
    togglePage (page) {
      ...
      if (this.dataTable.options.onPageChanged) {
        this.dataTable.options.onPageChanged(this.currentPage)
      }
    },
    sortBy (column) {
  ...
  computed: {
    ...
    pagination () {
      let newList = this.getPageRows(this.filteredRows, this.currentPage, this.dataTable.options.pageCount)
      // console.log(newList)
      return newList
    }
  },
      ...
      rows.forEach((row, index) => {
        for (let key in row) {
          ...
          row[key] = Object.assign({
            editable: column ? column.editable : '',
            editing: false,
            tmpValue: ''
          }, row[key])
        }
